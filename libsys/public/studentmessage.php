...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['send'])) {
    // Get sender username from the session
    $sender_username = $_SESSION['username'];

    // Get sender ID from the database
    $sender_id_query = "SELECT id FROM stdregister WHERE username = ?";
    ...
    $sender_id_row = $result->fetch_assoc();
    $sender_id = $sender_id_row['id'];

    // Get receiver email from the POST request
    if (empty($_POST['email'])) {
        $email_err = "Receiver email is required.";
    } else {
        $receiver_email = $_POST['email'];

        // Get receiver ID from the database
        $receiver_id_query = "SELECT id FROM stdregister WHERE email = ?";
        $stmt = $connection->prepare($receiver_id_query);
        $stmt->bind_param("s", $receiver_email);
        $stmt->execute();
        $result = $stmt->get_result();

        if ($result->num_rows == 0) {
            $email_err = "Receiver ID not found.";
        } else {
            $receiver_id_row = $result->fetch_assoc();
            $receiver_id = $receiver_id_row['id'];
        }
    }

    // Get message content from the POST request
    if (empty($_POST['content'])) {
        $content_err = "Message content is required.";
    } else {
        $message = $_POST['content'];
    }

    // Insert the message into the messages table if there are no errors
    if (empty($email_err) && empty($content_err)) {
        $insert_message_query = "INSERT INTO messages (senderid, receiverid, content, read_status) VALUES (?, ?, ?, 'no')";
        ...
        if ($stmt->execute()) {
            ...
            header("Location: studentmessage.php");
            exit();
        } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['mark_read'])) {
    ...
    $update_status_query = "UPDATE messages SET read_status = 'yes' WHERE id = ?";
    ...
    if ($stmt->execute()) {
        ...
        echo "Error marking message as read: " . $stmt->error;
    }
}
...
$receiver_id_query = "SELECT id FROM stdregister WHERE username = ?";
$stmt = $connection->prepare($receiver_id_query);
...
// Get messages sent to the logged-in user
$messages_query = "SELECT m.id, m.content, m.read_status, m.sent_at, s.username as sender_username
                   FROM messages m
                   JOIN stdregister s ON m.senderid = s.id
                   WHERE m.receiverid = ?";
...
<div class="container p-5">
    <div class="card p-5 mb-4 bg-dark text-white">
        <form action="messages.php" method="POST">
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control <?php if (!empty($content_err)) echo "is-invalid"; ?>" id="exampleInputText1" name="content">
                <div id="validationServer03Feedback" class="invalid-feedback">
                    <?php echo $content_err; ?>
                </div>
            </div>
            <button type="submit" name="send" class="btn btn-primary bg-dark">Send</button>
        </form>
    </div>
    ...
    <table class="table">
        ...
        if ($result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                echo "<tr>";
                echo "<td>" . htmlspecialchars($row["sender_username"]) . "</td>";
                echo "<td>" . htmlspecialchars($row["content"]) . "</td>";
                echo "<td>";
                if ($row["read_status"] == 'yes') {
                    echo "<span class='read-icon'>&#10003;</span>";
                } else {
                    echo "<span class='unread-icon'>&#10007;</span>";
                }
                echo "</td>";
                echo "<td>" . htmlspecialchars($row["sent_at"]) . "</td>";
                echo "<td>";
                if ($row["read_status"] == 'no') {
                    echo "<form action='messages.php' method='POST' style='display:inline;'>
