...
if (isset($_POST["register"])){
    ...
    if(empty($_POST["username"])){
      $username_err="Username it can't be empty!";
    }
    else{
    ...
    if(empty($_POST["email"])){
      $email_err="Email it can't be empty!";
    }
    else{
      $email=$_POST["email"];
    }
    ...
    if(empty($_POST["password"])){
      $password_err="Password it can't be empty!";
    }
    

    if(empty($_POST["cfmpassword"])){
      $cfmpassword_err="Password it can't be empty!";
    }
    else if($_POST["password"]!=$_POST["cfmpassword"]){
      $cfmpassword_err="Passwords aren't match";
    }
    else{
...
if(!empty($cfmpassword_err)){
  echo "is-invalid";
}
?>" id="exampleInputPassword1" name="cfmpassword">
            
            <div id="validationServer03Feedback" class="invalid-feedback">
            <?php
              echo $cfmpassword_err; 
            ?>
            </div>       
        </div>
